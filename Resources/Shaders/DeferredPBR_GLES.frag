...
uniform sampler2D preintegratedBRDFLUT;
...
void main()
{
	...
	if (debug != 0) {
		vec3 debugCol;
		if (debug == 1) debugCol = fragPos;
		else if (debug == 2) debugCol = (normal.xyz + 1.0) * 0.5;
		else if (debug == 3) debugCol = albedo;
		else if (debug == 4) debugCol = vec3(roughness);
		else if (debug == 5) debugCol = vec3(metallic);
		color = vec4(GammaCorrection(debugCol), 1.0);
		return;
	}
	*/
	// PBR
	vec3 lightDir = normalize(vec3(0.8));

	vec3 viewDir = normalize(camPosition - fragPos);
	vec3 halfwayDir = normalize(lightDir + viewDir);

	// Directional BRDF
	vec3 spec = SpecularBRDF(vec3(1), roughness, normal, halfwayDir, lightDir, viewDir);

	vec3 finalColor = spec + IBL(normal, viewDir, roughness, metallic, albedo, enviroment, preintegratedBRDFLUT);
